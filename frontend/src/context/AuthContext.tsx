...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { void load(); }, []);
  const login = async (email: string, password: string) => { await api.post('/auth/login', { email, password }); queryClient.clear(); await load(true); };
  const logout = async () => { await api.post('/auth/logout'); queryClient.clear(); setSession(null); };
