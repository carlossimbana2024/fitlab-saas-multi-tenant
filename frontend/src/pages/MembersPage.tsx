...
type Member = { id: string; status: string; joined_at?: string; profiles?: Profile };
...
type InviteForm = { fullName: string; email: string; phone: string };

const emptyInvite: InviteForm = { fullName: '', email: '', phone: '' };
...
export function MembersPage() {
  ...
  const visibleMembers = useMemo(() => {
    ...
    return (members.data ?? []).filter((member) => `${member.profiles?.full_name ?? ''} ${member.profiles?.phone ?? ''}`.toLocaleLowerCase('es').includes(needle));
  }, [members.data, search]);
  const invite = useMutation({
    mutationFn: async () => api.post('/members/invite', { fullName: inviteForm.fullName, email: inviteForm.email, phone: inviteForm.phone.trim() || null, defaultLocationId: session?.gymUser?.default_location_id ?? null }),
    onSuccess: async () => {
      setInviteOpen(false);
      setInvitedMessage(`Invitación enviada a ${inviteForm.email}.`);
      setInviteForm(emptyInvite);
      await queryClient.invalidateQueries({ queryKey: ['members'] });
    },
  });

  return <div className="page"><div className="page-heading"><div><p className="eyebrow">COMUNIDAD</p><h1>Miembros</h1><p>Administra las personas que forman parte de tu gimnasio.</p></div><button className="primary" onClick={() => setInviteOpen(true)}><UserPlus size={18}/>Invitar miembro</button></div>
    {invitedMessage && <div className="alert success">{invitedMessage}</div>}
    <section className="panel"><div className="table-tools"><div className="search"><Search/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nombre o teléfono…"/></div><span>{visibleMembers.length} miembros</span></div>{members.isLoading ? <div className="empty"><LoaderCircle className="spin"/>Cargando miembros…</div> : visibleMembers.length ? <div className="member-list">{visibleMembers.map((member) => <button className="member-row member-button" key={member.id} onClick={() => setSelectedId(member.id)}><span className="avatar">{member.profiles?.full_name?.slice(0, 2).toUpperCase() ?? 'FL'}</span><div><strong>{member.profiles?.full_name ?? 'Sin nombre'}</strong><small>{member.profiles?.phone ?? 'Sin teléfono'}</small></div><span className={`badge ${member.status}`}>{statusLabel[member.status] ?? member.status}</span></button>)}</div> : <div className="empty"><Users/><strong>No hay resultados</strong><span>{search ? 'Prueba con otro nombre o teléfono.' : 'Invita al primer miembro para empezar.'}</span></div>}</section>

    {inviteOpen && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setInviteOpen(false); }}><section className="modal" role="dialog" aria-modal="true"><div className="modal-heading"><div><p className="eyebrow">NUEVO MIEMBRO</p><h2>Enviar invitación</h2></div><button className="icon-button" onClick={() => setInviteOpen(false)}><X/></button></div><form className="checkout-form" onSubmit={(event: FormEvent) => { event.preventDefault(); invite.mutate(); }}><label>Nombre completo<input required minLength={2} maxLength={150} value={inviteForm.fullName} onChange={(event) => setInviteForm({ ...inviteForm, fullName: event.target.value })}/></label><label>Correo electrónico<input required type="email" value={inviteForm.email} onChange={(event) => setInviteForm({ ...inviteForm, email: event.target.value })}/></label><label>Teléfono opcional<input maxLength={40} value={inviteForm.phone} onChange={(event) => setInviteForm({ ...inviteForm, phone: event.target.value })}/></label><p className="form-note">Supabase enviará un correo para que el miembro configure su acceso. FitLab no crea ni conoce su contraseña.</p>{invite.isError && <div className="alert error">{apiMessage(invite.error)}</div>}<div className="modal-actions"><button type="button" className="ghost" onClick={() => setInviteOpen(false)}>Cancelar</button><button className="primary" disabled={invite.isPending}>{invite.isPending ? <><LoaderCircle className="spin"/>Enviando…</> : 'Enviar invitación'}</button></div></form></section></div>}

    {selectedId && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedId(null); }}><section className="modal member-detail" role="dialog" aria-modal="true"><div className="modal-heading"><div><p className="eyebrow">FICHA DEL MIEMBRO</p><h2>{detail.data?.member.profiles?.full_name ?? 'Miembro'}</h2><span className={`badge ${detail.data?.member.status}`}>{statusLabel[detail.data?.member.status ?? ''] ?? detail.data?.member.status}</span></div><button className="icon-button" onClick={() => setSelectedId(null)}><X/></button></div>{detail.isLoading ? <div className="empty"><LoaderCircle className="spin"/>Cargando ficha…</div> : detail.isError ? <div className="alert error">{apiMessage(detail.error)}</div> : detail.data && <MemberDetailContent data={detail.data}/>}</section></div>}
  </div>;
}
