...
export function AttendancesPage() {
  ...
  const memberNames = useMemo(() => new Map((members.data ?? []).map((member) => [member.id, member.profiles?.full_name ?? 'Miembro sin nombre'])), [members.data]);
