...
type StaffPayload = { staff: StaffMember[]; permissionCatalog: CatalogPermission[]; locations: Location[] };

const statusLabels = { invited: 'Invitación pendiente', active: 'Activo', suspended: 'Suspendido' };
...
export function StaffPage() {
  ...
  const selected = staffQuery.data?.staff.find((item) => item.id === selectedId) ?? null;
  const visibleStaff = useMemo(() => {
    const needle = search.trim().toLocaleLowerCase('es');
    if (!needle) return staffQuery.data?.staff ?? [];
    return (staffQuery.data?.staff ?? []).filter((item) =>
