...
type Receipt = { number: number; issuedAt: string; verificationToken: string; status: Payment['status']; amount: number; currency: string; paymentMethod: string; externalReference?: string | null; notes?: string | null; voidReason?: string | null; refundReason?: string | null; reversedAt?: string | null; concept: string; operationType: 'membership' | 'renewal'; gym: { name: string; legal_name?: string | null; email?: string | null; phone?: string | null; whatsapp_phone?: string | null; logo_url?: string | null }; location: { name: string; address?: string | null; city?: string | null; email?: string | null; phone?: string | null; whatsapp_phone?: string | null }; member: { name: string; phone?: string | null }; plan: { name: string }; coverage?: { starts_on: string; ends_on: string; original_amount?: number; discount_amount?: number } | null; registeredBy: { name: string } };
type PaymentMethod = 'cash' | 'bank_transfer' | 'external_card' | 'external_deuna' | 'other';
type CheckoutForm = { memberUserId: string; planId: string; membershipId: string; rewardId: string; paymentMethod: PaymentMethod; externalReference: string; notes: string };
...
const initialForm: CheckoutForm = { memberUserId: '', planId: '', membershipId: '', rewardId: '', paymentMethod: 'cash', externalReference: '', notes: '' };
...
export function MembershipsPage() {
  ...
  const [checkoutResult, setCheckoutResult] = useState<{ receipt_number: number | null; coverage_starts_on: string; coverage_ends_on: string } | null>(null);
  ...
  const loyalty = useQuery({ queryKey: ['loyalty', session?.gymUser?.gym_id, session?.gymUser?.id, form.memberUserId], enabled: role === 'owner' && open && Boolean(form.memberUserId && form.membershipId), queryFn: async () => (await api.get<LoyaltyData>(`/loyalty/members/${form.memberUserId}`)).data });
  const rewards = (loyalty.data?.rewards ?? []).filter((r) => r.status === 'available' && r.terms.reward_type !== 'product' && r.terms.location_id === session?.gymUser?.default_location_id);
  const selectedReward = rewards.find((r) => r.id === form.rewardId);
  const isFreeReward = selectedReward?.terms.reward_type === 'free_period';
  const checkout = useMutation({
    mutationFn: async () => (await api.post<{ checkout: typeof checkoutResult }>('/memberships/manual-checkout', {
      ...
      paymentMethod: isFreeReward ? 'cash' : form.paymentMethod,
      externalReference: isFreeReward ? null : form.externalReference.trim() || null,
      notes: form.notes.trim() || null,
    })).data.checkout,
    onSuccess: async (result) => {
      setOpen(false); setForm(initialForm); setCheckoutResult(result);
      await Promise.all(['memberships', 'payments', 'loyalty', 'dashboard'].map((key) => queryClient.invalidateQueries({ queryKey: [key] })));
    },
  });
  ...
  const selectMember = (memberUserId: string) => {
    ...
    setForm({ ...form, memberUserId, rewardId: '', membershipId: existing?.id ?? '', planId: existing?.plan_id ?? '' });
  };
  ...
  return <div className="page membership-page">
    ...
    {open && <Modal close={() => setOpen(false)} eyebrow={selectedMembership ? 'RENOVACIÓN' : 'NUEVO CONTRATO'} title={selectedMembership ? 'Renovar membresía' : 'Registrar pago manual'}><form className="checkout-form" onSubmit={submit}>
      ...
      {selectedMembership && <div className="form-note renewal-note">Se añadirá un nuevo período después de la cobertura existente. Sin recompensa se cobra el precio vigente del plan. El contrato conserva todo su historial.</div>}
      {selectedMembership && role === 'owner' && <label className="wide">Recompensa opcional<select value={form.rewardId} disabled={loyalty.isPending || loyalty.isError} onChange={(e) => setForm({ ...form, rewardId: e.target.value })}><option value="">Sin recompensa — precio normal</option>{rewards.map((r) => <option key={r.id} value={r.id}>{r.terms.name} · {rewardLabel(r.terms)}</option>)}</select><small>Solo premios reclamados y vigentes de tu sucursal. El backend volverá a validar la asistencia.</small>{loyalty.isError && <span className="alert error">{apiErrorMessage(loyalty.error)}</span>}</label>}
      {selectedReward && <div className="form-note wide">{isFreeReward ? `${selectedReward.terms.reward_value} mes(es) gratuito(s), sin generar pagos ni ingresos.` : `Descuento del ${selectedReward.terms.reward_value}% sobre el precio vigente. No acumulable con otro premio.`}</div>}
      {!isFreeReward && <><label>Método de pago<select value={form.paymentMethod} onChange={(event) => setForm({ ...form, paymentMethod: event.target.value as PaymentMethod })}><option value="cash">Efectivo</option><option value="bank_transfer">Transferencia bancaria</option><option value="external_card">Tarjeta externa</option><option value="external_deuna">DEUNA externo</option><option value="other">Otro</option></select></label>
      ...
      {checkout.isError && <div className="alert error">{apiErrorMessage(checkout.error)}</div>}<div className="modal-actions"><button type="button" className="ghost" onClick={() => setOpen(false)}>Cancelar</button><button className="primary" disabled={checkout.isPending || !form.memberUserId || !form.planId || Boolean(form.rewardId && (loyalty.isFetching || !selectedReward))}>{checkout.isPending ? <><LoaderCircle className="spin"/>Registrando…</> : selectedMembership ? 'Confirmar renovación' : 'Confirmar pago'}</button></div>
    </form></Modal>}
...
function ReceiptView({ receipt }: { receipt: Receipt }) {
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      y += 24;
      if (Number(receipt.coverage?.discount_amount) > 0) {
        pdf.text(`Precio: ${money(Number(receipt.coverage!.original_amount), receipt.currency)} · Descuento: ${money(Number(receipt.coverage!.discount_amount), receipt.currency)}`, left, y);
        y += 7;
      }
